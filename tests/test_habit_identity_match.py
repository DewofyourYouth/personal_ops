...
def test_match_key_normalises_punctuation_and_schedule():
    """Habit match keys ignore punctuation and parenthesized schedule annotations."""
...
def test_identity_matches_despite_trailing_period(tmp_path):
    """Identity updates match stored habit names even when typed without punctuation."""
...
def test_cue_matches_despite_schedule_annotation(tmp_path):
    """Cue updates match habits even when the user omits the stored schedule text."""
    store = _store(tmp_path)
...
def test_unknown_name_still_returns_none(tmp_path):
    """Unknown habit names return None instead of mutating another habit."""
    store = _store(tmp_path)
...
def test_identities_accumulate_and_dedupe(tmp_path):
    """Habit identities accumulate across updates without duplicate values."""
    store = _store(tmp_path)
...
def test_remove_one_identity_keeps_the_rest(tmp_path):
    """Removing one identity leaves the habit's other identities in place."""
    store = _store(tmp_path)
...
def test_identity_shared_across_habits(tmp_path):
    """The same identity value can be attached to multiple habits."""
    store = _store(tmp_path)
    store.add_identities(PROTEIN, ["healthy"])
...
def test_denormalised_cache_tracks_join(tmp_path):
    """The legacy habits.identity column mirrors the many-to-many identity join."""
...
def test_existing_single_identity_is_migrated_to_join(tmp_path):
    """Existing single-column identity values are migrated into the join table."""
