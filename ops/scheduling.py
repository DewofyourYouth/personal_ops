...
def start(log_dir: str, jobs: dict, *, plan_hour: int, plan_minute: int,
    ...
    scheduler.add_job(jobs["remind_upcoming"], "interval", seconds=600,
                      id="remind_upcoming", replace_existing=True)
    scheduler.add_job(jobs["check_reminders"], "interval", seconds=60,
                      id="check_reminders", replace_existing=True)
    scheduler.add_job(jobs["daily_digest"], "cron", hour=22, minute=30,
